...
class EditAssetsController extends GetxController {
  ...
  /// CALL THIS FROM UI
  Future<void> initWithAsset(String id) async {
    assetId = id;

    await getDropdown();      // 1️⃣ dropdowns
    await getAsset(assetId);  // 2️⃣ asset
    setData(); // Ensures correct selection state in edit mode and prevents multiselect search from mutating selected items.
              // setData();
    await getSubAsset();      // 3️⃣ now assetData exists
    await getExpectedResults();
  }


  Future<void> loadInitialData() async {
    // ✅ Load dropdown data first (must be awaited)
    await getDropdown();
  ...
  Future<void> getDropdown() async {
    final response =
    await ApisRequests().makeGetRequest(ApisFile().client_sa);

    if (response.statusCode == 200) {
      final List<dynamic> dropDownValueData =
  ...
  Future getSubAsset() async {
    final response =
    await ApisRequests().makeGetRequest(getSubAssetUrl());

    if (response.statusCode == 200) {
      List<SubAsset> subAssedata =
      (json.decode(response.body)['data'] as List)
          .map((e) => SubAsset.fromJson(e))
          .toList();

      subAssetData.assignAll(
        subAssedata
            .map(
              (e) => MultiSelectItem<Typedata>(
            Typedata(
      ...
      // Preselect from asset
      if ((assetData.value?.component_sub_assets ?? []).isNotEmpty) {
        for (var id in assetData.value!.component_sub_assets) {
          final index =
          subAssetData.indexWhere((e) => e.value.typeId == id);
          if (index != -1) {
  ...
  Future getExpectedResults() async {
    ...
    if (response.statusCode == 200) {
      List<ExpectedModel> allExpected =
      (json.decode(response.body)['data'] as List)
          .map((e) => ExpectedModel.fromJson(e))
          .toList();

      expectedData.assignAll(
        allExpected
      ...
      if ((assetData.value?.result_data ?? []).isNotEmpty) {
        for (var ex in assetData.value!.result_data) {
          final index = expectedData.indexWhere(
                  (e) => e.value.type_id == ex.type_id);
          if (index != -1) {
            selectExpectedResults.add(expectedData[index].value);
          }
        }
      }
  ...
  Future<bool> refreshOrderExpected(List<dynamic> newOrderList) async {
    ...
    for (var exdata in newOrderList) {
      if ((expectedData
          .any((element) => element.value.type_id == exdata.type_id))) {
        var index = expectedData
            .indexWhere((element) => element.value.type_id == exdata.type_id);
  ...
  Future<bool> refreshOrderSubAsset(List<dynamic> newOrderList) async {
    ...
    for (var subAsset in newOrderList) {
      if ((subAssetData
          .any((element) => element.value.typeId == subAsset.typeId))) {
        var index = subAssetData
            .indexWhere((element) => element.value.typeId == subAsset.typeId);
        selectSubAsset.add(subAssetData[index].value);
      }
    }
    selectSubAsset.refresh();
  ...
  void setData() {
    final asset = assetData.value;
    if (asset == null) return;

    // ✅ Only assign when dropdown lists are ready
    if (inspections.isNotEmpty && asset.component_inspectiontype != null) {
      selectInspectionType.value = findIndex(
        inspections,
        asset.component_inspectiontype!.type_id,
      );
    }

    if (uoms.isNotEmpty && asset.component_uom != null) {
      selectUom.value = findIndex(
        uoms,
        asset.component_uom!.type_id,
      );
    }

    if (standardArr.isNotEmpty && asset.standard_certificate_id != null) {
      selectStandard.value = findIndex(
        standardArr,
        asset.standard_certificate_id!.type_id,
      );
      selectStandard.refresh();
    }

    if (bodyTestReportArr.isNotEmpty &&
        asset.notified_body_certificate_id != null) {
      selectBodyTestrprt.value = findIndex(
        bodyTestReportArr,
        asset.notified_body_certificate_id!.type_id,
      );
      selectBodyTestrprt.refresh();
    }

    if (articleArr.isNotEmpty &&
        asset.article_11b_certificate_id != null) {
      select11bArticle.value = findIndex(
        articleArr,
        asset.article_11b_certificate_id!.type_id,
      );
      select11bArticle.refresh();
    }

    if (assetTypeArr.isNotEmpty && asset.component_type != null) {
      selectAssetType.value = findIndex(
        assetTypeArr,
        asset.component_type!.type_id,
      );
      selectAssetType.refresh();
    }

    if (assetCategoryArr.isNotEmpty && asset.component_category != null) {
      selectAssetCategory.value = findIndex(
        assetCategoryArr,
        asset.component_category!.type_id,
      );
      selectAssetCategory.refresh();
    }

    if (assetTemplateArr.isNotEmpty &&
        asset.component_template_type != null) {
      selectAssetTemplate.value = findIndex(
        assetTemplateArr,
        asset.component_template_type!.type_id,
      );
      selectAssetTemplate.refresh();
    }
  }
...
class _EditAssetsState extends State<EditAssets> {
  ...
  void initState() {
    ...
    // ✅ Load asset AFTER controller is ready
    controller.initWithAsset(widget.assetId); // ✅ CORRECT
  }
                  ...
                  ResponsiveRowColumnItem(
                    ...
                    child: Container(
                      ...
                      child: controller.refreshExpectResult.value
                        ...
                        children: [
                          Expanded(
                            child: MultiSelectDialogField(
                              searchable: true,
                              reorderble: true,
                              // Ensures correct selection state in edit mode and prevents multiselect search from mutating selected items.
                              // 🔥 FIX: pass COPY, not Rx reference
                              items: List<MultiSelectItem>.from(
                                  controller.expectedData),
                              ...
                              title: Text(
                                getStringTranslate("expected_result") + " *",
                              ),
                              ...
                              onConfirm: (results) {
                                ...
                                controller.selectExpectedResults.value = merged;
                                controller.selectExpectedResults.refresh();
                              },
                            ),
                          ),
                          Container(
                              ...
                              ontap: () async {
                                ...
                                await showDialog<List<dynamic>>(
                                  ...
                                  builder: (BuildContext context) {
                                    return ReorderableListDialog<String>(
                                      items:
                                      controller.selectExpectedResults.value,
                                    );
                                  },
                        ...
                        child: const Text("Expected Results Loading..."),
                      ),
                    ),
                  ),
                      ...
                      children: [
                        Expanded(
                          child: MultiSelectDialogField(
                            ...
                            //// Handles edit-mode preselection and prevents multiselect search from overwriting existing selections.
                            onConfirm: (results) {
                              final merged = {
                                ...controller.selectSubAsset.value,
                                ...results,
                              ...
                              controller.selectSubAsset.value = merged;
                              controller.selectSubAsset.refresh();
                            },
                          ),
                        ),
                        Container(
  ...
  editAsset() async {
    ...
    Map datapost = {
      ...
      "component_template_type": controller.selectAssetTemplate.value != null
          ? controller.selectAssetTemplate.value?.typeId
          : "",
